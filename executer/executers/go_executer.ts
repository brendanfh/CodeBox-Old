...
export class GoExecuter extends BaseExecuter {
    public async execute(path: string, input_file: string, time_limit: number): Promise<ExecutionResult> {
        ...
        bash_shell.stdin.end(`cat ${input_file} | timeout -s SIGKILL ${time_limit / 1000.0} go run ${path}`);
